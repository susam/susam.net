...
(defun fill-cm-block (blck params)
  (let* ((author (aget "author" params))
         ...
         (cm-url (if (string= cm-name author)
                     ...
                     (aget "url" blck))))
    (aput "cm-author" (if cm-url (fstr "<a href=\"~a\">~a</a>" cm-url cm-name) cm-name) blck)
    (aput "cm-class" (if (string= cm-name author) "author" "visitor") blck)
...
(defun fill-cm (cm-doc doc-map params)
  (aput "blocks" (mapp #'fill-cm-block (aget "blocks" cm-doc) params) cm-doc)
  (let* ((cm-slug (doc-slug cm-doc))
         (cm-doc-info (hget cm-slug doc-map))
...
(defun fill-head (doc includes params)
...
(defun main ()
  ...
  (let* ((params (list (cons "index" "")
         ...
         (layouts (mapp #'fill-layout (read-layouts "layout/")))
         (includes (read-includes "includes/"))
         (zones (aget "zones" config))
         (all-docs (mapp #'fill-path (read-docs "content/tree/") params))
    ...
    ;; Fill documents.
    (setf ren-docs (mapp #'fill-render ren-docs zones params))
    (setf cm-docs (mapp #'fill-cm-doc cm-doc doc-map params))
    ;; Fill imports.
    (setf ren-docs (mapp #'fill-head ren-docs includes params))
    (setf cm-docs (mapp #'fill-cm cm-docs includes params))
